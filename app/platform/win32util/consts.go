...
import (
	...
	"gioui.org/app/platform/operatingsystem"
...
func init() {
	if IsWindowsVersionAtLeast(10, 0, 18334) {
		...
		uxtheme := mylog.Check2(windows.LoadLibrary("uxtheme.dll"))
		procAllowDarkModeForWindow := mylog.Check2(windows.GetProcAddressByOrdinal(uxtheme, uintptr(133)))
		AllowDarkModeForWindow = func(hwnd HWND, allow bool) uintptr {
			var allowInt int32
			if allow {
				allowInt = 1
			}
			ret, _ := mylog.Check3(syscall.SyscallN(procAllowDarkModeForWindow, uintptr(hwnd), uintptr(allowInt)))
			return ret
		}

		// SetPreferredAppMode is only available on Windows 10+
		procSetPreferredAppMode := mylog.Check2(windows.GetProcAddressByOrdinal(uxtheme, uintptr(135)))
		SetPreferredAppMode = func(mode int32) uintptr {
			ret, _ := mylog.Check3(syscall.SyscallN(procSetPreferredAppMode, uintptr(mode)))
			return ret
		}
...
func GetModuleHandle(value uintptr) uintptr {
	result, _ := mylog.Check3(procGetModuleHandle.Call(value))
	return result
}

func GetMessage(msg *MSG) uintptr {
	rt, _ := mylog.Check3(procGetMessageW.Call(uintptr(unsafe.Pointer(msg)), 0, 0, 0))
	return rt
}

func PostMessage(hwnd HWND, msg uint32, wParam, lParam uintptr) uintptr {
	ret, _ := mylog.Check3(procPostMessage.Call(
		uintptr(hwnd),
		uintptr(msg),
		wParam,
		lParam))
...
func ShellNotifyIcon(cmd uintptr, nid *NOTIFYICONDATA) bool {
	ret, _ := mylog.Check3(procShellNotifyIcon.Call(cmd, uintptr(unsafe.Pointer(nid))))
	return ret == 1
}

func IsDialogMessage(hwnd HWND, msg *MSG) uintptr {
	ret, _ := mylog.Check3(procIsDialogMessage.Call(uintptr(hwnd), uintptr(unsafe.Pointer(msg))))
	return ret
}

func TranslateMessage(msg *MSG) uintptr {
	ret, _ := mylog.Check3(procTranslateMessage.Call(uintptr(unsafe.Pointer(msg))))
	return ret
}

func DispatchMessage(msg *MSG) uintptr {
	ret, _ := mylog.Check3(procDispatchMessage.Call(uintptr(unsafe.Pointer(msg))))
	return ret
}
