...
func SetTheme(hwnd uintptr, useDarkMode bool) {
	if SupportsThemes() {
		...
		dwmSetWindowAttribute(HWND(hwnd), attr, unsafe.Pointer(&winDark), unsafe.Sizeof(winDark))
	}
}
...
func dwmSetWindowAttribute(hwnd HWND, dwAttribute DWMWINDOWATTRIBUTE, pvAttribute unsafe.Pointer, cbAttribute uintptr) {
	ret, _ := mylog.Check3(procDwmSetWindowAttribute.Call(
		uintptr(hwnd),
		uintptr(dwAttribute),
		uintptr(pvAttribute),
		cbAttribute))
...
func SetWindowCompositionAttribute(hwnd HWND, data *WINDOWCOMPOSITIONATTRIBDATA) bool {
	if procSetWindowCompositionAttribute != nil {
		ret, _ := mylog.Check3(procSetWindowCompositionAttribute.Call(
			uintptr(hwnd),
...
func SetWindowTheme(hwnd HWND, appName string, subIdList string) uintptr {
	...
	ret, _ := mylog.Check3(procSetWindowTheme.Call(
		uintptr(hwnd),
...
func IsCurrentlyHighContrastMode() bool {
	...
	res, _ := mylog.Check3(procSystemParametersInfo.Call(SPI_GETHIGHCONTRAST, uintptr(result.CbSize), uintptr(unsafe.Pointer(&result)), 0))
	if res == 0 {
		return false
	}
