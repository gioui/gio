...
func (w *window) draw(sync bool) {
	...
	// Process embed view positions and hit regions.
	w.processEmbedView()
}
...
// processEmbedView updates the external regions for hit testing, window region, and positions external views.
func (w *window) processEmbedView() {
	current, lost := w.w.EmbeddedRegions()
	if len(current.Views) == 0 && len(lost) == 0 {
		return
	}
	...
	// Position current embed views first (behind Gio).
	for _, region := range current.Views {
		w.setEmbedViewPosition(region.View, int(region.Area.Min.X), int(region.Area.Min.Y),
			int(region.Area.Dx()), int(region.Area.Dy()))
	}
	// Hide lost views.
	for _, lostView := range lost {
		w.setEmbedViewPosition(lostView.View, 0, 0, 0, 0)
	}

	if len(current.Views) > 0 {
		windows.SetWindowPos(w.gioHwnd, windows.HWND_TOP, 0, 0, 0, 0, windows.SWP_NOMOVE|windows.SWP_NOSIZE|windows.SWP_NOACTIVATE)
	}

	// Avoid rect reconstruction
	if slices.Equal(w.externalRegions.Pass, current.Pass) && slices.Equal(w.externalRegions.Blockers, current.Blockers) {
		return
	}

	w.externalRegions.Pass = append(w.externalRegions.Pass[:0], current.Pass...)
	w.externalRegions.Blockers = append(w.externalRegions.Blockers[:0], current.Blockers...)

	w.updateWindowRegion(width, height)
}

// setEmbedViewPosition sets the position and size of an embedded Win32 window.
// The embed window is positioned and placed behind Gio.
func (w *window) setEmbedViewPosition(viewID uintptr, x, y, width, height int) {
	hwnd := syscall.Handle(viewID)
	...
	// Position the window and place it behind Gio (HWND_BOTTOM).
	// Gio will be brought to top after all embed views are positioned.
	windows.SetWindowPos(hwnd, windows.HWND_BOTTOM, int32(x), int32(y), int32(width), int32(height), windows.SWP_NOACTIVATE|windows.SWP_SHOWWINDOW)
}

// updateWindowRegion sets the window region on the Gio child window to create holes
// where external views should show through, while preserving areas where Gio draws
// on top (blockers).
func (w *window) updateWindowRegion(width, height int) {
	...
	// Subtract each pass region (hole) where embed views show through.
	...
	// Add back blocker regions where Gio draws on top of embed views.
	for _, blocker := range w.externalRegions.Blockers {
		// Clamp to window bounds.
		x1, y1 := int32(blocker.Min.X), int32(blocker.Min.Y)
		x2, y2 := int32(blocker.Max.X), int32(blocker.Max.Y)
		if x1 < 0 {
			x1 = 0
		}
		if y1 < 0 {
		...
		blockerRgn := windows.CreateRectRgn(x1, y1, x2, y2)
		if blockerRgn == 0 {
			continue
		}
		windows.CombineRgn(resultRgn, resultRgn, blockerRgn, windows.RGN_OR)
		windows.DeleteObject(blockerRgn)
	}
