...
var (
	...
	shader_cover_vert = backend.ShaderSources{
		...
		Uniforms: backend.UniformsReflection{
			Blocks:    []backend.UniformBlock{{Name: "Block", Binding: 0}},
			Locations: []backend.UniformLocation{{Name: "_66.transform", Type: 0x0, Size: 4, Offset: 0}, {Name: "_66.uvCoverTransform", Type: 0x0, Size: 4, Offset: 16}, {Name: "_66.uvTransform", Type: 0x0, Size: 4, Offset: 32}, {Name: "_66.z", Type: 0x0, Size: 1, Offset: 48}},
			Size:      52,
		},
		...
		/*
		   cbuffer Block : register(b0)
		   {
		       float4 _82_transform : packoffset(c0);
		       float4 _82_uvCoverTransform : packoffset(c1);
		       float4 _82_uvTransform : packoffset(c2);
		       float _82_z : packoffset(c3);
		   };
		   ...
		   void vert_main()
		   {
		       float4 param = float4((pos * _82_transform.xy) + _82_transform.zw, _82_z, 1.0f);
		       gl_Position = toClipSpace(param);
		       vUV = (uv * _82_uvTransform.xy) + _82_uvTransform.zw;
		       float3 _117[2];
		       fboTextureTransform(_117);
		       float3 fboTrans[2] = _117;
		       float3 param_1[2] = fboTrans;
		       float3 param_2 = float3(uv, 1.0f);
		       float3 uv3 = transform3x2(param_1, param_2);
		       vCoverUV = ((uv3 * float3(_82_uvCoverTransform.xy, 1.0f)) + float3(_82_uvCoverTransform.zw, 0.0f)).xy;
		   }
