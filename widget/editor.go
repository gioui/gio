...
func (e *Editor) processEvents(gtx layout.Context) iter.Seq[EditorEvent] {
	return func(yield func(EditorEvent) bool) {
		if len(e.pending) > 0 {
			out := e.pending[0]
		...
		defer func() {
			...
			if selStart != afterSelStart || selEnd != afterSelEnd {
				...
				//} else {
				//	ev = SelectEvent{}
				if !yield(SelectEvent{}) { //todo test
					return
				}
				//ok = true
			}
		...
		for editorEvent := range e.processKey(gtx) {
			if !yield(editorEvent) {
				return
			}
		}
...
func (e *Editor) processPointer(gtx layout.Context) iter.Seq[EditorEvent] {
	return func(yield func(EditorEvent) bool) {
		...
		for evt := range e.clicker.Update(gtx.Source) {
...
func (e *Editor) processPointerEvent(gtx layout.Context, ev event.Event) iter.Seq[EditorEvent] {
	return func(yield func(EditorEvent) bool) {
		switch evt := ev.(type) {
...
func (e *Editor) processKey(gtx layout.Context) iter.Seq[EditorEvent] {
	return func(yield func(EditorEvent) bool) {
		if e.text.Changed() {
			return
		}
		...
		for ke := range gtx.Event(filters...) {
			...
			case key.Event:
				...
				e.scrollCaret = true
				e.scroller.Stop()
				for editorEvent := range e.command(gtx, ke) {
					if !yield(editorEvent) {
						return
					}
				}
...
func (e *Editor) command(gtx layout.Context, k key.Event) iter.Seq[EditorEvent] {
	return func(yield func(EditorEvent) bool) {
		direction := 1
		...
		if k.Modifiers.Contain(key.ModShortcut) {
			...
			case "Z":
				if !e.ReadOnly {
					if k.Modifiers.Contain(key.ModShift) {
						for editorEvent := range e.redo() {
							if !yield(editorEvent) {
								return
							}
						}
					} else {
						for editorEvent := range e.undo() {
							if !yield(editorEvent) {
								return
							}
						}
...
// false.
func (e *Editor) Update(gtx layout.Context) iter.Seq[EditorEvent] {
	return func(yield func(EditorEvent) bool) {
		e.initBuffer()
...
// e.historyIdx.
func (e *Editor) undo() iter.Seq[EditorEvent] {
	return func(yield func(EditorEvent) bool) {
		e.initBuffer()
...
// e.historyIdx.
func (e *Editor) redo() iter.Seq[EditorEvent] {
	return func(yield func(EditorEvent) bool) {
		e.initBuffer()
